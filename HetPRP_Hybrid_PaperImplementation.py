...
def cb(model, where):
    ...
    if where == GRB.Callback.MIPSOL:
        ...
        for t in model._periods:
            ...
            x_arcs_dict = {(a[0],a[1]):{"capacity":x_arcs[a],"vehicle":a[3]} for a in x_arcs if a[2]==t and a[3] not in model._homok and a[3]>0}
...
def apply_mip_start_from_construction(main_model, construction_model, christofides_routes):
        ...
        for (period, vehicle), (route, distance) in christofides_routes.items():
            ...
            if main_model._det_vehicles == 4:
                if vehicle ==1:
                     vehicle = 2
                if vehicle == 2:
                    if (period,vehicle) not in routes_homo_k:
                        routes_homo_k[(period,vehicle)] = 0.0
            if vehicle==-2:
                routes_homo_k[(period,vehicle)] = 0.0
            ...
            for i in range(len(route) - 1):
                ...
                if (node_from, node_to, period, vehicle) in x_var_mapping:
                    if vehicle in main_model._homok:
                        if node_from==0:
                            qi = 0
                            if vehicle == 2 or vehicle==-2:
                                routes_homo_k[period,vehicle] += 1.0
                        else:
    ...
    for period,vehicle in routes_homo_k:
        main_vars[f"k[{period},{vehicle}]"].start = routes_homo_k[period,vehicle]
        print(f" Set k[{period},{vehicle}] = {routes_homo_k[period,vehicle]}")
...
def run_model2(filepath, setting, results=[]):
    ...
    model_name = f'model4_HPRP_comb_{model_type}_{cost_type}'
    if MIPstart == True:
    ...
    if BINOM:
        m.addConstrs((var_q[i,t,k]   <= min(inv_cap[i],Q2,sum(daily_demand[i,l] for l in periods[t-1:]))*var_z[i,t,k] for i in customers for t in periods for k in vehicles if k not in homo_k), name="Min quantity delivered only if customer is visited in same period")

        m.addConstrs((quicksum(var_q[i,t,k] for i in customers) <= Q2*var_z[0,t,k] for k in vehicles if k not in homo_k for t in periods), name="Capacity Constraints")
    else:
    ...
    if BINOM:
        if stoch==True:
            if NEG_OUT_PREMIUM==False:
                ...
                if Q==Q2:
                    m.addConstrs((var_k[t,-2]   <= var_z[0,t,max(vehicles)] for t in periods), name="vehicle_symmetry_vehicle3")
    else:
    ...
    m.addConstrs((var_x[i,j,t,k]+var_x[j,i,t,k]  <= var_z[i,t,k] for i in customers for j in customers if j!=i for t in periods for k in vehicles if (k>0 or k==-2)), name="subtours of size 2")
    ...
    def solution_to_single_csv_with_coordinates(solution_tuples, c_matrix, dist_matrix, coordinates,
        ...
        folder_path = os.path.join(base_path, f"{model_name}_{costtype}_{instance_type_id}_Torres2FleetsTestAllSameCap_{experiment_date}")
    ...
    df.to_csv(f"results/model4_16_threads_MIPstart_med{det_vehicles}_{recourse_gamma}_CrowdDrivers_twoFleets_Torres_QEqual.csv")
    #df.to_csv("results/model4_test_crowd_routes_2cap.csv")
    env.close()
    print(Q,Q2)
