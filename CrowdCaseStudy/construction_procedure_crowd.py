...
def create_cp_mip(file_path,recourse_gamma = 3, stoch=True,het_model=False, binom=False, binom_p=0):
    print(file_path)
    ...
    if stoch == True:
        if num_vehicles == 2:
            #we assume deterministic vehicles
    ...
    if binom:    
        m.addConstrs((var_q[i,t,k]   <= min(inv_cap[i],Q2,sum(daily_demand[i,l] for l in periods[t-1:]))*var_z[i,t,k] for i in customers for t in periods for k in vehicles if k>=0), name="Min quantity delivered only if customer is visited in same period")
        ...
        m.addConstrs((quicksum(var_q[i,t,k] for i in customers) <= Q2*var_z[0,t,k] for k in vehicles if k>=0 for t in periods), name="Capacity Constraints")
        ...
        m.addConstrs((var_z[i,t,k]   <= quicksum(var_z[i,t,k-1] for i in customers) for i in customers for t in periods for k in vehicles if k>1), name="vehicle_symmetry_vehicle3")
        m.addConstrs((var_z[i,t,k]   <= quicksum(var_z[i,t,k+1] for i in customers) for i in customers for t in periods for k in vehicles if k<-2), name="vehicle_symmetry_vehicle3")    
    else:
...
def generate_initial_solution(file_path, recourse_gamma,stoch=True, het_model=False, binom=False,binom_p=0):
         m,model_env = create_cp_mip(file_path,recourse_gamma=recourse_gamma,stoch = stoch, het_model = het_model, binom=binom, binom_p=binom_p)
         m.optimize()
