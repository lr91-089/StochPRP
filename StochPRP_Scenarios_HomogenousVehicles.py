...
def run_model(file_path,settings, results=[]):
    ...
    if VARIABLE_COST==1:
            routing_costs = fixed_own*quicksum(var_x[0,j,t] for j in customers for t in periods)+quicksum(r[i]*var_z[i,t] for i in customers for t in periods)+quicksum(c[i,j] * var_x[i, j,t] for i in nodes for j in nodes if i!=j for t in periods) 
    elif VARIABLE_COST==2:
            routing_costs = fixed_crowd*quicksum(var_x[0,j,t] for j in customers for t in periods)+quicksum(r[i]*var_z[i,t] for i in customers for t in periods)+quicksum(c[i,j]*variable_crowd_factor * var_x[i, j,t] for i in nodes for j in nodes if i!=j for t in periods) 
    else:
    ...
    if stoch==True:
        ...
        if VARIABLE_COST>0:
            recourse_costs = quicksum(probabilities[s]*fixed_own*(var_x[0,j,t]-var_alpha[0,j,t,s]) for j in customers for t in periods for s in probabilities)+quicksum(r[i]*probabilities[s]*(var_z[i,t]-var_beta[i,t,s]) for i in customers for t in periods for s in probabilities)+quicksum(probabilities[s]*c[i,j] * (var_x[i, j,t] -var_alpha[i,j,t,s]) for i in nodes for j in nodes if i!=j for t in periods for s in probabilities)
            unused_routing_costs = quicksum(probabilities[s]*fixed_crowd*(var_x[0,j,t]-var_alpha[0,j,t,s]) for j in customers for t in periods for s in probabilities)+quicksum(r[i]*probabilities[s]*(var_z[i,t]-var_beta[i,t,s]) for i in customers for t in periods for s in probabilities)+quicksum(probabilities[s]*c[i,j] * variable_crowd_factor* (var_x[i, j,t] -var_alpha[i,j,t,s]) for i in nodes for j in nodes if i!=j for t in periods for s in probabilities)
    ...
    m.addConstrs((quicksum(var_x[0,j,t] for j in customers) == quicksum(var_x[j,0,t] for j in customers) for t in periods), name="degree constraints at plant")

    
    m.addConstrs((quicksum(var_x[0,j,t] for j in customers) <= num_vehicles for i in customers for t in periods), name="vehicle limit")
    ...
    if stoch==True:
        ...
        m.addConstrs((var_alpha[i,j,t,s]  <= var_x[i,j,t] for (i,j) in dist for t in periods for s in probabilities), name="1.20")
        
        m.addConstrs((var_beta[i,t,s]  <= var_z[i,t] for i in customers for t in periods for s in probabilities), name="1.21")
    ...
    m._periods = periods
    if STOCH_VALUE==True: 
        route_name = file_path.split("/")[-1].strip(".dat")+"_rg2.0_f100_fc50_vc0.5_ri0"+"_sol"
    ...
    def parse_solution_to_routes_no_vehicle_with_artificial_vehicle(solution_tuples):
        ...
        for node1, node2, period in solution_tuples:
            routes_by_t[period].append((node1, node2))
        
        ordered_routes = {}
        
        for period, x_arcs in routes_by_t.items():
            tours = []
            vehicle_id = 1
            ...
            for arcs in x_arcs:
                if arcs[0] == 0 and arcs not in processed_arcs:   
                    ...
                    ordered_routes[(period, vehicle_id)] = sorted_tour
                    vehicle_id += 1
    ...
    def solution_to_single_csv_no_vehicle_artificial_vehicle(solution_tuples, c_matrix, dist_matrix, coordinates,
        ...
        filename = f"{instance_name}_rg{recourse_gamma}_f{fixed_own}_fc{fixed_crowd}_vc{variable_crowd_factor}_ri{r_payment}_VOCD_sol.csv"
        ...
        with open(filepath, 'w') as f:
            ...
            for (period, vehicle), route_edges in routes.items():
                for sequence_order, (node1, node2) in enumerate(route_edges, 1):
                    ...
                    edge_dist1 = c_matrix[node1, node2] if hasattr(c_matrix, '__getitem__') else c_matrix(node1, node2)
    ...
    #df.to_csv("results/model1_homogeneousK_16_threads_integral_Med3_CrodwDrivers_determ_VSS_computation_calc_RoutesVCD_Torres2.csv")
    df.to_csv("results/REcompute_OnlyOwnDriver_routesVOCD.csv")
    env.close()
...
#filenames =["./Instances/Data_Test/MVPRP_C15_P3_V3_I1.dat"]
